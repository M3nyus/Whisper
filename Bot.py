...
class Demo:
    def __init__(self, roomdId, player=None, recorder=MediaBlackhole(), loop=None):
        if loop is None:
            try:
...
if __name__ == "__main__":
    ...
    roomId = Demo.getRoomId()
    def should_end():
        demo.close()
    try:
        demo = Demo(roomdId=roomId, player=player, recorder=recorder, loop=loop)
